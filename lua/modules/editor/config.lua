...
function config.vista() vim.g.vista_default_executive = 'coc' end
...
function config.format()
  require("format").setup {
        ...
        cmd = {
          function(file)
            return string.format(
                       "lua-format --tab-width=2 --indent-width=2 -i %s", file)
          end
        }
      }
    },
    ...
    rust = {
      {cmd = {function(file) return string.format("rustfmt %s", file) end}}
    }
  }
end
